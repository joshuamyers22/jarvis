...
check "zone_region_boundary" {
  assert {
    ...
    error_message = "The development zone must belong to the configured region."
  }
}
...
module "platform" {
  ...
  db_backup_start_time              = "05:00"
  db_backup_retained_count          = 8
  db_transaction_log_retention_days = 7
  db_maintenance_day                = 2
